...
public class BarView extends View {
    ...
    private int sideStyle = BarViewSideStyle.ROUNDED;
    private int sideTextStyle = BarViewSideTextStyle.ONE_SIDED;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        valueSignCenter = -1;
        int segmentsSize = bars == null ? 0 : bars.size();
        if (segmentsSize > 0) {
            for (int segmentIndex = 0; segmentIndex < segmentsSize; segmentIndex++) {
                Bar bar = bars.get(segmentIndex);

                drawSegment(canvas, bar, segmentIndex, segmentsSize);
            }
        } else {
    ...
    private void drawEmptySegment(Canvas canvas) {
        ...
        if (barRoundingRadius > singleSegmentWidth / 2) {
            sideStyle = BarViewSideStyle.NORMAL;
        }

        segmentRect.set(rectBounds);

        switch (sideStyle) {
            case BarViewSideStyle.ROUNDED:
    ...
    private void drawSegment(Canvas canvas, Bar bar, int segmentIndex, int segmentsSize) {
        ...
        } else if (value != null && (value >= bar.getMinValue() && value < bar.getMaxValue() || (isRightSegment && bar.getMaxValue() == value))) {
            float valueSignCenterPercent = (value - bar.getMinValue()) / (bar.getMaxValue() - bar.getMinValue());
            valueSignCenter = (int) (segmentLeft + getPaddingLeft() + valueSignCenterPercent * singleSegmentWidth);
        }

        fillPaint.setColor(bar.getColor());
        ...
        if (showSegmentText) {
            String textToShow;
            if (bar.getCustomText() != null) {
                textToShow = bar.getCustomText();
            } else {
                if (isLeftSegment || isRightSegment) {
                    if (isLeftAndRight || sideTextStyle == BarViewSideTextStyle.TWO_SIDED) {
                        textToShow = String.format("%s - %s", formatter.format(bar.getMinValue()), formatter.format(bar.getMaxValue()));
                    } else if (isLeftSegment) {
                        textToShow = String.format("<%s", formatter.format(bar.getMaxValue()));
                    } else {
                        textToShow = String.format(">%s", formatter.format(bar.getMinValue()));
                    }
                } else {
                    textToShow = String.format("%s - %s", formatter.format(bar.getMinValue()), formatter.format(bar.getMaxValue()));
                }
        ...
        if (showDescriptionText) {
            ...
            drawTextCentredInRectWithSides(canvas, descriptionTextPaint, bar.getDescriptionText(), segmentRect.left, segmentRect.bottom, segmentRect.right, segmentRect.bottom + descriptionBoxHeight);
        }
    }
    ...
    public static Builder builder(Context context) {
        return new BarView(context).new Builder();
    }
    ...
    public class Builder {
        ...
        public Builder segments(List<Bar> bars) {
            BarView.this.bars = bars;
            return this;
        }

        public Builder unit(String unit) {
            BarView.this.unit = unit;
            BarView.this.createValueTextLayout();
            return this;
        }

        public Builder value(Float value) {
            BarView.this.value = value;
            BarView.this.createValueTextLayout();
            return this;
        }

        public Builder valueSegment(Integer valueSegment) {
            BarView.this.valueSegment = valueSegment;
            BarView.this.createValueTextLayout();
            return this;
        }

        public Builder valueSegmentText(String valueSegmentText) {
            BarView.this.valueSegmentText = valueSegmentText;
            BarView.this.createValueTextLayout();
            return this;
        }

        public Builder gapWidth(int gapWidth) {
            BarView.this.gapWidth = gapWidth;
            return this;
        }

        public Builder barHeight(int barHeight) {
            BarView.this.barHeight = barHeight;
            return this;
        }

        public Builder showDescriptionText(boolean showDescriptionText) {
            BarView.this.showDescriptionText = showDescriptionText;
            return this;
        }

        public Builder valueSignSize(int width, int height) {
            BarView.this.valueSignWidth = width;
            BarView.this.valueSignHeight = height;
            return this;
        }

        public Builder valueSignColor(int valueSignColor) {
            BarView.this.valueSignColor = valueSignColor;
            return this;
        }

        public Builder showSegmentText(boolean showText) {
            BarView.this.showSegmentText = showText;
            return this;
        }

        public Builder sideStyle(int sideStyle) {
            BarView.this.sideStyle = sideStyle;
            return this;
        }

        public Builder emptySegmentColor(int emptySegmentColor) {
            BarView.this.emptySegmentColor = emptySegmentColor;
            return this;
        }

        public Builder sideTextStyle(int sideTextStyle) {
            BarView.this.sideTextStyle = sideTextStyle;
            return this;
        }

        public Builder descriptionTextSize(int descriptionTextSize) {
            BarView.this.descriptionTextSize = descriptionTextSize;
            return this;
        }

        public Builder segmentTextSize(int segmentTextSize) {
            BarView.this.segmentTextSize = segmentTextSize;
            return this;
        }

        public Builder valueTextSize(int valueTextSize) {
            BarView.this.valueTextSize = valueTextSize;
            return this;
        }

        public Builder descriptionTextColor(int descriptionTextColor) {
            BarView.this.descriptionTextColor = descriptionTextColor;
            return this;
        }

        public Builder segmentTextColor(int segmentTextColor) {
            BarView.this.segmentTextColor = segmentTextColor;
            return this;
        }

        public Builder valueTextColor(int valueTextColor) {
            BarView.this.valueTextColor = valueTextColor;
            return this;
        }

        public Builder descriptionBoxHeight(int descriptionBoxHeight) {
            BarView.this.descriptionBoxHeight = descriptionBoxHeight;
            return this;
        }

        public BarView build() {
            return BarView.this;
        }
